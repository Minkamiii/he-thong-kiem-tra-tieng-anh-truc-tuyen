...
public class SubmitService {
    ...
    // get all test (for test choose) that have been done
    ...
    // get all submits (history) of user
    ...
    // get submits (for task choose) by user ID and test ID
    public ApiResponse getSubmitByUserAndTest(String id_user, String id_test, Integer page) {
        ...
        Page<Submit> submitPage = submitRepository.findByUserIdAndTestIdOrderBySubmitDayDESC(id_user, id_test,
                pageable);
        ...
        Submit.Type type = submitPage.getContent().get(0).getType();

        if (type == Submit.Type.LISTENING) {
            Integer max_listening = submitRepository.findMaxListeningCorrect(id_test, id_user);
            Integer min_listening = submitRepository.findMinListeningCorrect(id_test, id_user);
            ...
            Integer max_reading = submitRepository.findMaxReadingCorrect(id_test, id_user);
            Integer min_reading = submitRepository.findMinReadingCorrect(id_test, id_user);
    ...
    // for admin to get progress of a test
    public ApiResponse getProgressForAdmin(String test_id) {

        ApiResponse response = new ApiResponse();
        List<Submit> submits = submitRepository.findByTestId(test_id);

        if (submits == null || submits.isEmpty()) {
            response.setMessage("No submits found for test ID: " + test_id);
            response.setStatus(404);
            return response;
        } else {

            int totalUsersDone = submitRepository.countDistinctUsersByTestId(test_id);
            Submit.Type type = submits.get(0).getType();
            Map<String, Object> data = new HashMap<>();

            if (type == Submit.Type.LISTENING) {
                Integer max_listening = submitRepository.findMaxListeningCorrectOfTest(test_id);
                Integer min_listening = submitRepository.findMinListeningCorrectOfTest(test_id);
                data.put("highest", max_listening);
                data.put("lowest", min_listening);
                ...
                Integer max_listening = submitRepository.findMaxReadingCorrectOfTest(test_id);
                Integer min_listening = submitRepository.findMinReadingCorrectOfTest(test_id);
                data.put("highest", max_listening);
                data.put("lowest", min_listening);
                List<String> combined = answerService.getQuestionNoCorrectUser(test_id);
            ...
            data.put("totalUsersDone", totalUsersDone);
            response.setData(data);
        }

        response.setMessage("Get all information of test done for admin successfully");
        response.setStatus(200);
        return response;
