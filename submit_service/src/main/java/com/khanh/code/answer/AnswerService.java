...
import com.khanh.code.api_response.AiRequest;
import com.khanh.code.api_response.AiResponse;
...
public class AnswerService {
    ...
    public Object CreateAnswer(Object userAnswer, Submit submit, Question question) {
        ...
        if (answer_type == Answer.Type.FILL) {
            ...
            String aiGrade = sendAnswerToAi(answerEssay);
            if(aiGrade == null){
                ((Answer_Essay) answer).setAiRecommend("No AI recommendation");
            }
            else {
                ((Answer_Essay) answer).setAiRecommend(aiGrade);
            }
            System.out.println("AI grade: " + aiGrade);
        }
    ...
    public List<String> getQuestionNoCorrectUser(String id_test) {
        List<String> fillQuestions = answerRepository.findAllQuestionsAllWrong_Fill(id_test);
        List<String> choiceQuestions = answerRepository.findAllQuestionsAllWrong_Choice(id_test);
        fillQuestions.addAll(choiceQuestions);
        return fillQuestions;
    }

    public String sendAnswerToAi(String answerEssay) {
        
        String url= AIServiceUrl + "grade";
        try{
            AiRequest aiRequest = new AiRequest();
            aiRequest.setAnswer(answerEssay);
            ResponseEntity<AiResponse> aiResponse = restTemplate.postForEntity(url, aiRequest, AiResponse.class);
                if(aiResponse.getStatusCode().is2xxSuccessful()){
                    AiResponse aiRes = aiResponse.getBody();
                    return aiRes.getResponse();
                }
                else{
                    return null;
                }
            ...
            System.out.println("Error when calling AI service: " + e.getMessage());
            return null;
        }
    }
}